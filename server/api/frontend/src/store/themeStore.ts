...
export const useThemeStore = create<ThemeState & ThemeAction>()(
   persist(
      ...
      {
         name: 'theme',
         version: 2, // Increment version to apply new default mode
         // After rehydration, update mode to match new default if needed
         onRehydrateStorage: () => state => {
            if (state && state.mode !== inititialThemeState.mode) {
               // Update mode to match the new default (dark mode)
               // This will be saved on next state change
               state.mode = inititialThemeState.mode;
            }
         },
      }
   )
