...
export interface DropdownMenuProps extends CommonProps, DropdownToggleSharedProps, DropdownSubItemSharedProps {
   activeKey?: string;
               ...
               {isMounted && (
                  <FloatingPortal>
                     <FloatingFocusManager context={context} modal={false} initialFocus={isNested ? -1 : 0} returnFocus={!isNested}>
                        <div ref={refs.setFloating} style={{ ...floatingStyles, zIndex }} {...getFloatingProps()} className="outline-hidden">
                           <ul className={classNames('dropdown-menu min-w-[160px]', menuClass)} style={styles}>
                              {children}
                           </ul>
                        </div>
                     </FloatingFocusManager>
