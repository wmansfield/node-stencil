...
const StackedSideNavMini = (props: StackedSideNavMiniProps) => {
   const { onChange, routeKey, activeKeys, onSetActiveKey, direction, navigationTree, userAuthority, mode, selectedMenu, t, ...rest } = props;
   ...
   const defaultActiveKeys = activeKeys && activeKeys.length > 0 ? activeKeys : isEmpty(selectedMenu) ? [] : [includedRouteTree.key];
   ...
   return (
      <div {...rest}>
         ...
         >
            <Logo imgClass="max-h-10" mode={mode} type="streamline" className={SIDE_NAV_CONTENT_GUTTER} />
         </Link>
