...
const VerticalMenuContent = (props: VerticalMenuContentProps) => {
   const { collapsed, routeKey, navigationTree = [], onMenuItemClick, direction = themeConfig.direction, translationSetup, userAuthority } = props;
            ...
            {navTree.map(nav => (
               <Fragment key={nav.key}>
                  ...
                  {nav.type === NAV_ITEM_TYPE_COLLAPSE && (
                     ...
                     >
                        {nav.subMenu && nav.subMenu.length > 0 && renderNavigation(nav.subMenu, nextCascade, true)}
                     </VerticalCollapsedMenuItem>
                  )}
                  {nav.type === NAV_ITEM_TYPE_TITLE && (
                     <AuthorityCheck userAuthority={userAuthority} authority={nav.authority}>
                        <MenuGroup key={nav.key} label={t(nav.translateKey) || nav.title}>
                           {nav.subMenu && nav.subMenu.length > 0 && renderNavigation(nav.subMenu, cascade, false)}
                        </MenuGroup>
                     </AuthorityCheck>
                  )}
               </Fragment>
