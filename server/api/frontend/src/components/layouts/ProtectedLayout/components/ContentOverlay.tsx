   ...
   return (
      <LayoutBase
         ...
         pageContainerReassemble={({
            ...
            <>
               <div className={classNames(defaultClass, 'sm:px-0 px-4', header?.title ? '-mt-60' : '-mt-60 sm:-mt-40')}>
                  <main className="h-full">
                     <div className="dark container mx-auto relative">
                        <PageContainerHeader
                           ...
                           customeHeader={() => (
                              <>
                                 {header?.title && typeof header?.title === 'string' && <h2 className="mb-2">{header.title}</h2>}
                                 {header?.description && <p className="text-base text-white opacity-60">{header.description}</p>}
                              </>
                           )}
                        />
                     </div>
                     <PageContainerBody pageContainerType={'contained'}>
                        {pageBackgroundType === 'plain' ? (
                           <div className={classNames('bg-white dark:bg-gray-900 rounded-2xl', pageContainerGutterClass)}>{children}</div>
                        ) : (
                           children
                        )}
                        <PageContainerFooter
      ...
      >
         <div className="flex flex-auto min-w-0">
            <div className="flex flex-col flex-auto min-h-screen min-w-0 relative w-full">
               <Header
                  container
                  className={classNames('fixed', isSticky ? 'shadow-sm dark:shadow-2xl' : 'bg-transparent dark')}
