...
type PredictionMediumPickerMultiProps = {
   className?: string;
   id?: string;
   value?: string[];
   null_display?: string;
...
function PredictionMediumPickerMulti(props: PredictionMediumPickerMultiProps) {
   ...
   const predictionmediums: Option[] = [
      { value: '0', display: 'text_completion' },
      { value: '1', display: 'text_open' },
      { value: '2', display: 'image_sd' },
      { value: '3', display: 'image_hd' },
      { value: '4', display: 'audio_short' },
      { value: '5', display: 'audio_long' },
   ];

   if (null_display && null_display.length > 0) {
      predictionmediums.unshift({
         value: '',
   ...
   const findOptionLabel = (value?: string): string => {
      return predictionmediums.find(t => t.value === value)?.display ?? '';
   };
   ...
   return (
      <Select
         ...
         options={predictionmediums}
         className={classNames(className)}
      />
   );
}

export default PredictionMediumPickerMulti;