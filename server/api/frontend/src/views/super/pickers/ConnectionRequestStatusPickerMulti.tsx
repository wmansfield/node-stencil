...
type ConnectionRequestStatusPickerMultiProps = {
   className?: string;
   id?: string;
   value?: string[];
   null_display?: string;
...
function ConnectionRequestStatusPickerMulti(props: ConnectionRequestStatusPickerMultiProps) {
   ...
   const connectionrequeststatuss: Option[] = [
      { value: '0', display: 'Pending' },
      { value: '1', display: 'Accepted' },
      { value: '2', display: 'Declined' },
      { value: '3', display: 'Retracted' },
      { value: '4', display: 'Expired' },
   ];

   if (null_display && null_display.length > 0) {
      connectionrequeststatuss.unshift({
         value: '',
   ...
   const findOptionLabel = (value?: string): string => {
      return connectionrequeststatuss.find(t => t.value === value)?.display ?? '';
   };
   ...
   return (
      <Select
         ...
         options={connectionrequeststatuss}
         className={classNames(className)}
      />
   );
}

export default ConnectionRequestStatusPickerMulti;