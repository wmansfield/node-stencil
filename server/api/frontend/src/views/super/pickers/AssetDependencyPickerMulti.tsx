...
type AssetDependencyPickerMultiProps = {
   className?: string;
   id?: string;
   value?: string[];
   null_display?: string;
...
function AssetDependencyPickerMulti(props: AssetDependencyPickerMultiProps) {
   ...
   const assetdependencies: Option[] = [
      { value: '0', display: 'account' },
      { value: '1', display: 'widget' },
   ];

   if (null_display && null_display.length > 0) {
      assetdependencies.unshift({
         value: '',
   ...
   const findOptionLabel = (value?: string): string => {
      return assetdependencies.find(t => t.value === value)?.display ?? '';
   };
...
export default AssetDependencyPickerMulti;