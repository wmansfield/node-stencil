...
type CompletionNodeKindPickerMultiProps = {
   className?: string;
   id?: string;
   value?: string[];
   null_display?: string;
...
function CompletionNodeKindPickerMulti(props: CompletionNodeKindPickerMultiProps) {
   ...
   const completionnodekinds: Option[] = [
      { value: '0', display: 'source' },
      { value: '1', display: 'target' },
      { value: '2', display: 'date' },
      { value: '3', display: 'action' },
      { value: '4', display: 'emphasis' },
      { value: '5', display: 'text' },
      { value: '6', display: 'image' },
      { value: '7', display: 'audio' },
   ];

   if (null_display && null_display.length > 0) {
      completionnodekinds.unshift({
         value: '',
   ...
   const findOptionLabel = (value?: string): string => {
      return completionnodekinds.find(t => t.value === value)?.display ?? '';
   };
   ...
   return (
      <Select
         ...
         options={completionnodekinds}
         className={classNames(className)}
      />
   );
}

export default CompletionNodeKindPickerMulti;