...
type CommentStatusPickerMultiProps = {
   className?: string;
   id?: string;
   value?: string[];
   null_display?: string;
...
function CommentStatusPickerMulti(props: CommentStatusPickerMultiProps) {
   ...
   const commentstatuss: Option[] = [
      { value: '0', display: 'active' },
      { value: '1', display: 'removed' },
      { value: '2', display: 'moderated' },
   ];

   if (null_display && null_display.length > 0) {
      commentstatuss.unshift({
         value: '',
   ...
   const findOptionLabel = (value?: string): string => {
      return commentstatuss.find(t => t.value === value)?.display ?? '';
   };
   ...
   return (
      <Select
         ...
         options={commentstatuss}
         className={classNames(className)}
      />
   );
}

export default CommentStatusPickerMulti;