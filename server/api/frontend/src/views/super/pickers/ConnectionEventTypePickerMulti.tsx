...
type ConnectionEventTypePickerMultiProps = {
   className?: string;
   id?: string;
   value?: string[];
   null_display?: string;
...
function ConnectionEventTypePickerMulti(props: ConnectionEventTypePickerMultiProps) {
   ...
   const connectioneventtypes: Option[] = [
      { value: '0', display: 'Connected via Invite' },
      { value: '1', display: 'Connected via Request' },
      { value: '2', display: 'Disconnected' },
      { value: '3', display: 'Connected via QR' },
   ];

   if (null_display && null_display.length > 0) {
      connectioneventtypes.unshift({
         value: '',
   ...
   const findOptionLabel = (value?: string): string => {
      return connectioneventtypes.find(t => t.value === value)?.display ?? '';
   };
   ...
   return (
      <Select
         ...
         options={connectioneventtypes}
         className={classNames(className)}
      />
   );
}

export default ConnectionEventTypePickerMulti;