...
type InvitationStatusPickerMultiProps = {
   className?: string;
   id?: string;
   value?: string[];
   null_display?: string;
...
function InvitationStatusPickerMulti(props: InvitationStatusPickerMultiProps) {
   ...
   const invitationstatuss: Option[] = [
      { value: '0', display: 'Pending' },
      { value: '1', display: 'Claimed' },
      { value: '2', display: 'Retracted' },
   ];

   if (null_display && null_display.length > 0) {
      invitationstatuss.unshift({
         value: '',
   ...
   const findOptionLabel = (value?: string): string => {
      return invitationstatuss.find(t => t.value === value)?.display ?? '';
   };
   ...
   return (
      <Select
         ...
         options={invitationstatuss}
         className={classNames(className)}
      />
   );
}

export default InvitationStatusPickerMulti;