...
type AuthorizationMethodPickerMultiProps = {
   className?: string;
   id?: string;
   value?: string[];
   null_display?: string;
...
function AuthorizationMethodPickerMulti(props: AuthorizationMethodPickerMultiProps) {
   ...
   const authorizationmethods: Option[] = [
      { value: '0', display: 'initial_registration' },
      { value: '1', display: 'email_confirmation' },
      { value: '2', display: 'existing_device_approval' },
   ];

   if (null_display && null_display.length > 0) {
      authorizationmethods.unshift({
         value: '',
   ...
   const findOptionLabel = (value?: string): string => {
      return authorizationmethods.find(t => t.value === value)?.display ?? '';
   };
...
export default AuthorizationMethodPickerMulti;