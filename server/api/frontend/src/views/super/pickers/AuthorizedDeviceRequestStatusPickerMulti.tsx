...
type AuthorizedDeviceRequestStatusPickerMultiProps = {
   className?: string;
   id?: string;
   value?: string[];
   null_display?: string;
...
function AuthorizedDeviceRequestStatusPickerMulti(props: AuthorizedDeviceRequestStatusPickerMultiProps) {
   ...
   const authorizeddevicerequeststatuss: Option[] = [
      { value: '0', display: 'pending' },
      { value: '1', display: 'approved' },
      { value: '2', display: 'expired' },
      { value: '3', display: 'rejected' },
   ];

   if (null_display && null_display.length > 0) {
      authorizeddevicerequeststatuss.unshift({
         value: '',
   ...
   const findOptionLabel = (value?: string): string => {
      return authorizeddevicerequeststatuss.find(t => t.value === value)?.display ?? '';
   };
   ...
   return (
      <Select
         ...
         options={authorizeddevicerequeststatuss}
         className={classNames(className)}
      />
   );
}

export default AuthorizedDeviceRequestStatusPickerMulti;