...
type DeletionStatusPickerMultiProps = {
   className?: string;
   id?: string;
   value?: string[];
   null_display?: string;
...
function DeletionStatusPickerMulti(props: DeletionStatusPickerMultiProps) {
   ...
   const deletionstatuss: Option[] = [
      { value: '0', display: 'Pending Confirmation' },
      { value: '1', display: 'Confirmed' },
      { value: '2', display: 'Processing' },
      { value: '3', display: 'Completed' },
      { value: '4', display: 'Failed' },
      { value: '5', display: 'Cancelled' },
   ];

   if (null_display && null_display.length > 0) {
      deletionstatuss.unshift({
         value: '',
   ...
   const findOptionLabel = (value?: string): string => {
      return deletionstatuss.find(t => t.value === value)?.display ?? '';
   };
   ...
   return (
      <Select
         ...
         options={deletionstatuss}
         className={classNames(className)}
      />
   );
}

export default DeletionStatusPickerMulti;