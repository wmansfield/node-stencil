...
type ReportStatusPickerMultiProps = {
   className?: string;
   id?: string;
   value?: string[];
   null_display?: string;
...
function ReportStatusPickerMulti(props: ReportStatusPickerMultiProps) {
   ...
   const reportstatuss: Option[] = [
      { value: '0', display: 'Open' },
      { value: '1', display: 'Reviewed' },
      { value: '2', display: 'Resolved' },
      { value: '3', display: 'Dismissed' },
   ];

   if (null_display && null_display.length > 0) {
      reportstatuss.unshift({
         value: '',
   ...
   const findOptionLabel = (value?: string): string => {
      return reportstatuss.find(t => t.value === value)?.display ?? '';
   };
   ...
   return (
      <Select
         ...
         options={reportstatuss}
         className={classNames(className)}
      />
   );
}

export default ReportStatusPickerMulti;