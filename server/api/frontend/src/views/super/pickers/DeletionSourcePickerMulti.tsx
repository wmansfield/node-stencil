...
type DeletionSourcePickerMultiProps = {
   className?: string;
   id?: string;
   value?: string[];
   null_display?: string;
...
function DeletionSourcePickerMulti(props: DeletionSourcePickerMultiProps) {
   ...
   const deletionsources: Option[] = [
      { value: '0', display: 'User' },
      { value: '1', display: 'Apple S2S' },
      { value: '2', display: 'Inbound Email' },
   ];

   if (null_display && null_display.length > 0) {
      deletionsources.unshift({
         value: '',
   ...
   const findOptionLabel = (value?: string): string => {
      return deletionsources.find(t => t.value === value)?.display ?? '';
   };
   ...
   return (
      <Select
         ...
         options={deletionsources}
         className={classNames(className)}
      />
   );
}

export default DeletionSourcePickerMulti;