...
type PredictionKindPickerMultiProps = {
   className?: string;
   id?: string;
   value?: string[];
   null_display?: string;
...
function PredictionKindPickerMulti(props: PredictionKindPickerMultiProps) {
   ...
   const predictionkinds: Option[] = [
      { value: '0', display: 'prediction' },
      { value: '1', display: 'reveal' },
   ];

   if (null_display && null_display.length > 0) {
      predictionkinds.unshift({
         value: '',
   ...
   const findOptionLabel = (value?: string): string => {
      return predictionkinds.find(t => t.value === value)?.display ?? '';
   };
   ...
   return (
      <Select
         ...
         options={predictionkinds}
         className={classNames(className)}
      />
   );
}

export default PredictionKindPickerMulti;