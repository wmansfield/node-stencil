...
type AccountStatusPickerMultiProps = {
   className?: string;
   id?: string;
   value?: string[];
   null_display?: string;
...
function AccountStatusPickerMulti(props: AccountStatusPickerMultiProps) {
   ...
   const accountstatuss: Option[] = [
      { value: '-1', display: 'disabled' },
      { value: '0', display: 'enabled' },
   ];

   if (null_display && null_display.length > 0) {
      accountstatuss.unshift({
         value: '',
   ...
   const findOptionLabel = (value?: string): string => {
      return accountstatuss.find(t => t.value === value)?.display ?? '';
   };
   ...
   return (
      <Select
         ...
         options={accountstatuss}
         className={classNames(className)}
      />
   );
}

export default AccountStatusPickerMulti;