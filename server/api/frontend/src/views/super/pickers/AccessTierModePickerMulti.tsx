...
type AccessTierModePickerMultiProps = {
   className?: string;
   id?: string;
   value?: string[];
   null_display?: string;
...
function AccessTierModePickerMulti(props: AccessTierModePickerMultiProps) {
   ...
   const accesstiermodes: Option[] = [
      { value: '0', display: 'Active' },
      { value: '1', display: 'Grace' },
      { value: '2', display: 'Trial' },
   ];

   if (null_display && null_display.length > 0) {
      accesstiermodes.unshift({
         value: '',
   ...
   const findOptionLabel = (value?: string): string => {
      return accesstiermodes.find(t => t.value === value)?.display ?? '';
   };
   ...
   return (
      <Select
         ...
         options={accesstiermodes}
         className={classNames(className)}
      />
   );
}

export default AccessTierModePickerMulti;