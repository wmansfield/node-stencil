# CodeGenerator Build Script
# Builds both CLI and GUI as single-file executables
...
dotnet publish CodeGenerator.Cli -c $Configuration -r $Runtime --self-contained true /p:PublishSingleFile=true /p:DebugType=None /p:DebugSymbols=false -o $OutputDir
...
if ($LASTEXITCODE -eq 0) {
    ...
    if ($cliExe) {
        ...
        Write-Host "  Size: $([math]::Round($cliExe.Length / 1MB, 2)) MB" -ForegroundColor Gray
    }
} else {
...
dotnet publish CodeGenerator.Gui -c $Configuration -r $Runtime --self-contained true /p:PublishSingleFile=true /p:DebugType=None /p:DebugSymbols=false -o $OutputDir
...
if ($LASTEXITCODE -eq 0) {
    ...
    if ($guiExe) {
        ...
        Write-Host "  Size: $([math]::Round($guiExe.Length / 1MB, 2)) MB" -ForegroundColor Gray
    }
} else {
